    ...
    products_with_latest_revision as (
     ...
     -- Let's get all the addresses and keep the latest address_id, aws_account_id, and valid_from combination
	 -- We're transitioning from a bi-temporal data model to an uni-temporal data_model
     piifeed_with_uni_temporal_data as (
       ...
       (
         select
          *,
          ROW_NUMBER() OVER (PARTITION BY address_id, aws_account_id, valid_from ORDER BY from_iso8601_timestamp(update_date) desc) as row_num
         from
          piifeed
       )
       where
	 ...
	 -- For the purpose of the sales compensation report, we want to get the latest revision of an address + account_id pair
...
invoices_with_full_address as (
  ...
  from
    ...
    left join pii_with_latest_revision pii_data on pii_data.aws_account_id = invoice_amounts.payer_aws_account_id
