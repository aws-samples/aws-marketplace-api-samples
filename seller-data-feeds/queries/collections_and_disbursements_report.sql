...
-- ============================================================================
-- CTE: disbursed_amount_by_product_refactor
-- Filters to non-invoiced records
-- applies COALESCE defaults and rounding for the final output columns.
-- ============================================================================

disbursed_amount_by_product_refactor as (
select
    internal_buyer_invoice_line_item_surrogate_id_with_currency,
    count_distinct_pricing_currency,
    count_distinct_disbursement_currency,
    internal_buyer_invoice_line_item_surrogate_id,
    disbursement_id_or_invoiced as disbursement_id,
    end_user_account_id as internal_end_user_account_id,
    ...
    case
        when Agreement_ID is null then 'Not available'
        when Subscriber_Address_ID is null then 'Not provided'
    ...
    -- Procurement Info --
    ----------------------
    case when Offer_ID is null then 'Not available' else Offer_ID end as Offer_ID,
    case when agreement_id is null then 'Not available' when offer_set_id is null then 'Not applicable' else offer_set_id end as offer_set_id,
    ...
    case when Offer_ID is null then 'Not available' when Offer_Name is null and Offer_Target = 'Public' then 'Not applicable' else Offer_Name end as Offer_Name,
    -- offer target at time of invoice.,
    case when Offer_ID is null then 'Not available' else Offer_Target end as Offer_Target,
    case when Opportunity_Id is null then
             case when Offer_Target = 'Public' then 'Not applicable'
                  when Offer_Target is null and Agreement_Id is not null then 'Not applicable'
                  else null end else Opportunity_Id end as Opportunity_Id,
    case when Offer_Opportunity_Name is null then
             case when Offer_Target = 'Public' then 'Not applicable'
                  when Offer_Target is null and Agreement_Id is not null then 'Not applicable'
                  else null end
         else Offer_Opportunity_Name end as Offer_Opportunity_Name,
    case when Offer_Opportunity_Description is null then
             case when Offer_Target = 'Public' then 'Not applicable'
                  when Offer_Target is null and Agreement_Id is not null then 'Not applicable'
                  else null end
         else Offer_Opportunity_Description end as Offer_Opportunity_Description,
    case when Offer_target is null and Agreement_Id is not null and Agreement_Id <> 'Not available' then 'Public' else Offer_target end as offer_visibility,
    case
        when Reseller_AWS_Account_ID is null and Opportunity_Id is null then 'Not applicable'
        when recipient_account_id <> manufacturer_account_id and Reseller_AWS_Account_ID is null then 'Not applicable'
        else Reseller_AWS_Account_ID end as Reseller_AWS_Account_ID,
    ...
    case
        ...
        when Reseller_AWS_Account_ID is null and opportunity_id is null then 'Not applicable'
        when recipient_account_id <> manufacturer_account_id and Reseller_AWS_Account_ID is null then 'Not applicable'
        else Reseller_Company_Name end as Reseller_Company_Name,
    coalesce(Agreement_ID, 'Not available') as Agreement_ID,
    -- all agreement related data are surfaced as they were at time of invoice.
    case when Agreement_Id is null then 'Not available'
         when Agreement_Revision is null or Agreement_Revision = '' then 'Not provided'
         else Agreement_Revision end as Agreement_Revision,
    case when Agreement_Id is null then cast(null as TIMESTAMP WITH TIME ZONE) else Agreement_Start_Date end as Agreement_Start_Date,
    case when Agreement_Id is null then cast(null as TIMESTAMP WITH TIME ZONE) else Agreement_End_Date end as Agreement_End_Date,
    case when Agreement_Id is null then cast(null as TIMESTAMP WITH TIME ZONE) else Agreement_Acceptance_Date end as Agreement_Acceptance_Date,

    status,
    estimated_charges_currency_code,
    estimated_charges_net_amount,
    purchase_invoice_id,
    coalesce(listing_fee_invoice_id, 'Not applicable') as listing_fee_invoice_id,
    coalesce(resale_invoice_id, 'Not applicable') as resale_invoice_id,

    report_transaction_reference_id as transaction_reference_id,

    purchase_invoice_date as Invoice_Date,
    listing_fee_invoice_date as Charge_Invoice_Date,
    disbursement_date,
    last_disbursement_date,
    -- This FIRST unioned query filters where disbursement_id_or_invoiced <> '<invoiced>',
    -- so the bank_trace_id should either be provided (non-null) or 'Not available'.
    -- It should never be 'Not applicable' on a disbursement!
    coalesce(bank_trace_id, 'Not available') as disburse_bank_trace_id,
    coalesce(amazon_reference_id, 'Not available') as disbursement_reference_number,
    Usage_Period_Start_Date,
    Usage_Period_End_Date,
    Payment_Due_Date,
    disbursement_currency as Currency,
    disbursement_currency,
    broker_id,

    -- Amounts --
    -- Pricing Currency --
    -- Debugging Columns (invoice/disbursed amounts not invoiced _or_ disbursement amounts) --
    round(disbursement_gross_revenue_invoiced,2) as Gross_Revenue,
    round(disbursement_gross_refund_invoiced,2) as Gross_Refund,
    round(disbursement_infrastructure_netting_invoiced,2) as infrastructure_netting,
    round(disbursement_balance_adjustment_invoiced,2) as balance_adjustment,

    round(-1 * disbursement_gross_revenue_disbursed,2) as disbursed_gross_revenue,
    round(-1 * disbursement_gross_refund_disbursed,2) as disbursed_gross_refund,
    round(-1 * disbursement_infrastructure_netting_disbursed,2) as disbursed_infrastructure_netting,
    round(-1 * disbursement_balance_adjustment_disbursed,2) as disbursed_balance_adjustment,
    ...
    -- that was (or is) due to the seller, at the point when it was invoiced. At the point of invoicing,
    -- the net amount owed to the seller is usually POSITIVE, unless it is a refund.
    round(disbursement_seller_net_revenue, 2) as seller_net_revenue,
    round(disbursement_all_balance_impacting_invoiced,2) as all_balance_impacting_invoiced, -- should match seller_net_revenue
    -- End Debugging columns --


    round(-1 * disbursement_gross_revenue_this_disbursement_id_or_invoiced,2) as gross_revenue_this_disbursement_id,
    round(-1 * disbursement_gross_refund_this_disbursement_id_or_invoiced,2) as gross_refund_this_disbursement_id,
    round(-1 * disbursement_cogs_this_disbursement_id_or_invoiced,2) as cogs_this_disbursement_id,
    round(-1 * disbursement_cogs_refund_this_disbursement_id_or_invoiced,2) as cogs_refund_this_disbursement_id,
    round(-1 * disbursement_aws_rev_share_this_disbursement_id_or_invoiced,2) as listing_fee_this_disbursement_id,
    round(-1 * disbursement_aws_refund_share_this_disbursement_id_or_invoiced,2) as listing_fee_refund_this_disbursement_id,
    
    cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as listing_fee_numerator,   -- for debugging
    cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))  as listing_fee_denominator, -- for debugging
    length(replace(cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as varchar), '.', '')) as numerator_precision, -- for debugging
    round(
        case
            when cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6)) != 0 then
                case
                    -- This is a surprising workaround that is can only be reproduced using column values from a table
                    -- (not using literal values). If we divide two number that use the maximum precision (38),
                    -- then Redshift reduces the scale, just to be "safe", to reduce the likelihood over overlow,
                    -- resulting in unexpected results.
                    --
                    -- Using literals, Redshift detects the _actual_ precision and skips the precision reduction:
                    -- select (189.48)::numeric(38,6) / (-9474.23)::numeric(38,6) --> -0.0199995144724162
                    --
                    -- Using column values, division of the same numeric(38,6) values results in a scale of 4,
                    -- and (surprisingly) it does not matter how many casts to numeric(38,6) you include!
                    -- --> -0.0199
                    when length(replace(cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as varchar), '.', '')) <= 15 then
                        abs(
                            cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as decimal(15,6))/
                            cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))
                            )
                    when length(replace(cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as varchar), '.', '')) <= 20 then
                        abs(
                            cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as decimal(15,6))/
                            cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))
                            )
                    when length(replace(cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as varchar), '.', '')) <= 25 then
                        abs(
                            cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as decimal(15,6))/
                            cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))
                            )
                    when length(replace(cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as varchar), '.', '')) <= 30 then
                        abs(
                            cast(cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6)) as decimal(15,6))/
                            cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))
                            )
                        ...
                        abs(
                            cast((disbursement_aws_rev_share_this_disbursement_id_or_invoiced + disbursement_aws_refund_share_this_disbursement_id_or_invoiced) as decimal(38,6))/
                            cast((disbursement_gross_revenue_this_disbursement_id_or_invoiced + disbursement_gross_refund_this_disbursement_id_or_invoiced    ) as decimal(38,6))
                            )
                end
        ...
        6
    ) as listing_fee_percentage_this_disbursement_id,
    
    round(-1 * disbursement_aws_tax_share_this_disbursement_id_or_invoiced,2) as aws_tax_share_this_disbursement_id,
    round(-1 * disbursement_aws_tax_share_listing_fee_this_disbursement_id_or_invoiced,2) as aws_tax_share_listing_fee_this_disbursement_id,
    round(-1 * disbursement_aws_tax_share_refund_this_disbursement_id_or_invoiced,2) as aws_tax_share_refund_this_disbursement_id,
    round(-1 * disbursement_aws_tax_share_refund_listing_fee_this_disbursement_id_or_invoiced,2) as aws_tax_share_refund_listing_fee_this_disbursement_id,
    round(-1 * disbursement_seller_tax_share_this_disbursement_id_or_invoiced,2) as seller_tax_share_this_disbursement_id,
    round(-1 * disbursement_seller_tax_share_refund_this_disbursement_id_or_invoiced,2) as seller_tax_share_refund_this_disbursement_id,
    round(-1 * disbursement_other_seller_tax_share_this_disbursement_id_or_invoiced,2) as other_seller_tax_share_this_disbursement_id,
    round(-1 * disbursement_other_seller_tax_share_refund_this_disbursement_id_or_invoiced,2) as other_seller_tax_share_refund_this_disbursement_id,
    round(-1 * disbursement_infrastructure_netting_this_disbursement_id_or_invoiced,2) as infrastructure_netting_this_disbursement_id,

    -- There are only 6 old records having seller_rev_credit or aws_ref_fee_credit, all disbursed in 2013
    round(-1 * disbursement_seller_rev_credit_this_disbursement_id_or_invoiced) as seller_rev_credit,
    round(-1 * disbursement_aws_ref_fee_credit_this_disbursement_id_or_invoiced) as aws_ref_fee_credit,

    -- only reseller wholesale aws tax share are balance impacting
    case when is_isv_wholesale_invoice then round(0, 2) else round(-1 * disbursement_wholesale_aws_tax_share_this_disbursement_id_or_invoiced, 2) end as wholesale_aws_tax_share_this_disbursement_id,
    round(-1 * disbursement_wholesale_aws_tax_share_refund_this_disbursement_id_or_invoiced,2) as wholesale_aws_tax_share_refund_this_disbursement_id,
    round(-1 * disbursement_wholesale_seller_tax_share_this_disbursement_id_or_invoiced,2) as wholesale_seller_tax_share_this_disbursement_id,
    round(-1 * disbursement_wholesale_seller_tax_share_refund_this_disbursement_id_or_invoiced,2) as wholesale_seller_tax_share_refund_this_disbursement_id,
    round(-1 * disbursement_wholesale_other_seller_tax_share_this_disbursement_id_or_invoiced,2) as wholesale_other_seller_tax_share_this_disbursement_id,
    round(-1 * disbursement_wholesale_other_seller_tax_share_refund_this_disbursement_id_or_invoiced,2) as wholesale_other_seller_tax_share_refund_this_disbursement_id,

    round(-1 * disbursement_all_balance_impacting_this_disbursement_id_or_invoiced,2) as all_balance_impacting_this_disbursement_id, -- should match seller_net_revenue_this_disbursement_id

    round(-1 * disbursement_seller_net_revenue_this_disbursement_id_or_invoiced,2) as seller_net_revenue_this_disbursement_id,
    round(-1 * disbursement_seller_net_revenue_this_disbursement_id_or_invoiced,2) as seller_net_revenue_this_disbursement_id_if_disbursed,
    -- Each internal_buyer_invoice_line_item_surrogate_id_with_currency can have multiple partial disbursements.
    --
    -- The seller_net_revenue_this_disbursement_id_if_disbursed column has the amount of seller_net_revenue in _each_ partial disbursement.
    --
    -- We sum seller_net_revenue_this_disbursement_id_if_disbursed (for all partial disbursements for a given internal_buyer_invoice_line_item_surrogate_id_with_currency)
    -- to calculate the total disbursed seller_net_revenue for all partial disbursements.
    sum(round(-1 * disbursement_seller_net_revenue_this_disbursement_id_or_invoiced,2)) over (partition by internal_buyer_invoice_line_item_surrogate_id_with_currency) as sum_seller_net_revenue_this_disbursement_id_if_disbursed,

    round(disbursement_undisbursed_net_revenue,2) as undisbursed_net_revenue,

    round(-1 * disbursement_seller_net_revenue_this_disbursement_id_or_invoiced,2) as disbursement_seller_net_revenue_this_disbursement_id,
    round(-1 * disbursement_seller_net_revenue_this_disbursement_id_or_invoiced,2) as disbursement_seller_net_revenue_this_disbursement_id_if_disbursed,
    
    disbursement_flag,
    disbursement_flag_status as disbursement_status,

    -- This FIRST unioned query filters where disbursement_id_or_invoiced <> '<invoiced>',
    -- so the payment_due_date is always relevant, if it is not null:
    date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) as disbursement_time_in_days,

    case
        when date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) <= 0 then'Not due'
        when date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) >=1  and 
            date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) <=30 then '1-30 days late'
        when date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) >=31 and 
            date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) <=60 then '31-60 days late'
        when date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) >=61 and 
            date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) <=90 then '61-90 days late'
        when date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) >=91 and 
            date_diff('day', date_trunc('day', payment_due_date), date_trunc('day', disbursement_date)) <=120 then '91-120 days late'
        else '121+ days late'
    end as disbursement_period,

    coalesce(seller_issued_invoice_id, 'Not applicable') as seller_issued_invoice_id,            -- For deemed VAT (and future) where AWS is the recipient
    coalesce(seller_issued_invoice_variant, 'Not applicable') as seller_issued_invoice_variant,  -- For deemed VAT (and future) where AWS is the recipient
    
    
    manufacturer_aws_account_id as Manufacturer_AWS_Account_ID,
    Manufacturer_Encrypted_Account_ID,
    coalesce(Manufacturer_Company_Name, 'Not available') as Manufacturer_Company_Name,
    cast(null as varchar) as AR_Period,
    data_catalog

from
    line_items_with_window_functions_enrich_offer_product_address_name

where
    coalesce(disbursement_id_or_invoiced, '<null>') <> '<invoiced>'

  and last_disbursement_id is not null
  -- NOTE: The above one-line where clause is equivalent to the following one-line where clause:
  --and nvl(disbursement_status, '<null>'), <> 'Not disbursed'
),


-- ============================================================================
-- CTE: collections_and_disbursements_report
-- Renames columns for the final user-facing output and computes payment_term.
-- ============================================================================

collections_and_disbursements_report as (
select
    invoice_date, 
    payment_due_date,
    concat('Net ',
    ...
    disbursement_status,
    disbursement_date,
    disbursement_time_in_days,
    disburse_bank_trace_id,
    disbursement_reference_number,

    gross_revenue_this_disbursement_id as gross_revenue,
    gross_refund_this_disbursement_id as gross_refund,
    listing_fee_this_disbursement_id as listing_fee,
    listing_fee_refund_this_disbursement_id as listing_fee_refund,
    listing_fee_percentage_this_disbursement_id as listing_fee_percentage,
    seller_tax_share_this_disbursement_id as seller_tax_share,
    seller_tax_share_refund_this_disbursement_id as seller_tax_share_refund,
    aws_tax_share_listing_fee_this_disbursement_id as aws_tax_share_listing_fee,
    aws_tax_share_refund_listing_fee_this_disbursement_id as aws_tax_share_refund_listing_fee,
    cogs_this_disbursement_id as wholesale_cost,
    cogs_refund_this_disbursement_id as wholesale_cost_refund,
    wholesale_seller_tax_share_this_disbursement_id as wholesale_seller_tax_share,
    wholesale_seller_tax_share_refund_this_disbursement_id as wholesale_seller_tax_share_refund,
    wholesale_other_seller_tax_share_this_disbursement_id as wholesale_other_seller_tax_share,
    wholesale_other_seller_tax_share_refund_this_disbursement_id as wholesale_other_seller_tax_share_refund,
    wholesale_aws_tax_share_this_disbursement_id as wholesale_aws_tax_share,
    wholesale_aws_tax_share_refund_this_disbursement_id as wholesale_aws_tax_share_refund,
    seller_net_revenue_this_disbursement_id as seller_net_revenue,
    currency,
...
select *
from collections_and_disbursements_report
where payment_due_date >= date_add('DAY', -90, current_date)
--where payment_due_date between cast('2025-01-01' as TIMESTAMP WITH TIME ZONE) and cast('2026-03-01' as TIMESTAMP WITH TIME ZONE)
